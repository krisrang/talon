module CurrentUser
  def self.has_auth_cookie?(env)
    Talon.current_user_provider.new(env).has_auth_cookie?
  end

  def self.lookup_from_env(env)
    Talon.current_user_provider.new(env).current_user
  end
  ...
  def log_off_user(strict=false)
    current_user_provider.log_off_user(session,cookies,strict)
  end
  ...
  def current_user_provider
    @current_user_provider ||= Talon.current_user_provider.new(request.env)
  end
end
